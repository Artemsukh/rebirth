#!/usr/bin/env python3
"""Phase 9: place names for the language switcher (한국어, English, 日本語, Español).

- fills the ja and es columns of data/appdata.json LOC with CLDR region names (the ICU data
  bundled with Node, looked up by the iso2 column), plus the overrides below where CLDR gives
  a long official form or a name that differs from the one the UN data uses
- writes NAMES: continent (CONT) and subregion (SUB) names in en, ja and es, in the same order
  as the Korean CONT and SUB lists
...
# and the name Spanish speakers use for Côte d'Ivoire
OVERRIDE = {
...
def main():
    ...
    d['NAMES'] = {lang: {'CONT': CONT[lang], 'SUB': SUB[lang]} for lang in ('en', 'ja', 'es')}
    ...
    print('names for %d places in ja and es (CLDR %s), %d overrides' %
